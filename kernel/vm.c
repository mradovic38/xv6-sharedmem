...
// returns the address of the pte from pgdir that refers to the virtual address va
...
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
	...
	return &pgtab[PTX(va)]; // return the pte_t item (we take the bottom 10 bits of the logical address)
}

// makes pte's for virtual addresses starting with va that point to physical addresses starting with pa, va and size
// they do not have to be rounded by page
static int
...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
	...
	if(newsz >= 0x40000000) // exceeds MAX_REG_SZ
		return 0;
	if(newsz < oldsz)
		return oldsz;
